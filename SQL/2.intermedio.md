...
Se busca validar la comprensión del ciclo de vida de una consulta agregada. El candidato debe demostrar que sabe que `WHERE` filtra filas individuales y `HAVING` filtra grupos. Un error grave sería intentar colocar la condición de la suma en la cláusula `WHERE`.
...
### 4. Criterio de Evaluación del Entrevistador
El objetivo es evaluar la capacidad de estructurar joins múltiples y mantener la claridad en los alias de tabla. El entrevistador observa si el candidato elige el tipo de join correcto (`INNER JOIN`) y si es capaz de seguir el camino de las llaves foráneas a través de múltiples entidades.
...
### 4. Criterio de Evaluación del Entrevistador
Se evalúa el dominio de los "Anti-Joins". El candidato debe demostrar que sabe utilizar un `LEFT JOIN` junto con un filtro `IS NULL` para encontrar elementos sin relación. El uso de `NOT EXISTS` sería una alternativa válida y el entrevistador valoraría la capacidad del candidato para discutir ambas opciones en términos de rendimiento.
