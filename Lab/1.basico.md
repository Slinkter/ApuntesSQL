...
El entrevistador busca validar que el candidato comprende la diferencia entre la proyección (`SELECT`) y la filtración (`WHERE`). Un error crítico es intentar filtrar en el `SELECT` o no considerar la columna de estado (`discontinued`), lo que indicaría falta de atención a las reglas de negocio.
...
### 4. Criterio de Evaluación del Entrevistador
Se evalúa la capacidad de optimizar la extracción de datos. El uso de `LIMIT` junto a `ORDER BY` es un patrón común para rankings. El entrevistador observa si el candidato comprende que el ordenamiento es costoso y que el `LIMIT` permite al motor utilizar optimizaciones como Top-N Heapsort.
...
### 4. Criterio de Evaluación del Entrevistador
Este ejercicio detecta si el candidato domina la lógica trivalente de SQL. El error más común es intentar usar `region = NULL`, lo cual es técnicamente incorrecto y devuelve un conjunto vacío. El uso correcto de `IS NULL` y `COALESCE` demuestra madurez en el manejo de datos ausentes.
