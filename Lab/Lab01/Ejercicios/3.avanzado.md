...
> Base de datos: Northwind (PostgreSQL 15+) | 50 ejercicios | Temas: CTEs, Recursividad, Ventanas, LATERAL, Cohortes, Optimizacion
...
> **La analogía del resumen del torneo:**
> Imagina que juegas en un campeonato que dura meses y tu entrenador quiere saber qué jugadores hicieron más puntos en total.
> 
> En lugar de calcular los puntos cada vez que el entrenador mira la tabla (lo cual le daría dolor de cabeza), tú haces una hoja de cálculo intermedia o "borrador" (`WITH ventas_por_cliente AS (...)`) donde calculas el total de puntos y partidos de cada jugador.
> 
> Una vez que tienes ese borrador listo, el entrenador simplemente toma los nombres reales de los jugadores de la lista oficial de inscritos (`INNER JOIN customers`) y los junta con tu borrador para ver a los 10 mejores de forma rápida y limpia.
> Esto es una **CTE (Common Table Expression)**: una tabla borrador temporal que usas para simplificar tu consulta.
...
### Tip del Profesor
*   **gotcha de División por Cero:** Usamos `NULLIF(v.num_pedidos, 0)`. En matemáticas y bases de datos, dividir por cero destruye la consulta y da error. `NULLIF` convierte un `0` en `NULL`, y cualquier número dividido entre `NULL` da `NULL` en lugar de romper tu sistema en producción.
*   **Fórmula Comercial LTV:** Recuerda aplicar `(1 - discount)`. El descuento es un número real decimal (ej. `0.15` para 15%). Multiplicar por `(1 - discount)` descuenta el porcentaje de forma directa y matemáticamente exacta.
...
### Explicacion Sencilla
> **La analogía de las dos alcancías:**
> Imagina que quieres saber cuánto dinero neto te queda a fin de mes. Tienes dos alcancías:
> 1. La alcancía de ingresos, donde guardas cada moneda que ganas vendiendo limonada (`ingresos_mensuales`).
> 2. La alcancía de gastos de transporte, donde guardas los recibos del mototaxi que usaste para ir a comprar limones (`fletes_mensuales`).
> 
> A fin de mes, sumas el dinero de la alcancía de ingresos por mes, y por separado sumas los recibos de la alcancía de fletes por mes. Finalmente, juntas ambos resultados usando el mes en común (`LEFT JOIN`) y restas los gastos de los ingresos para ver tu ganancia neta.
> *Tener dos borradores independientes (`WITH` con dos comas) te permite ordenar tus pensamientos antes de hacer la resta final.*
...
### Tip del Profesor
*   **DATE_TRUNC y Casting:** `DATE_TRUNC('month', o.order_date)` trunca la fecha al primer día del mes (ej. `1997-05-15` se convierte en `1997-05-01 00:00:00`). Le agregamos `::date` al final para eliminar las horas y minutos innecesarios, ahorrando bytes en memoria y acelerando el `JOIN`.
*   **Tratamiento de Nulos con COALESCE:** Si en algún mes no se pagaron fletes, la suma dará `NULL`. Al restar `ingresos - NULL`, SQL daría `NULL` (lo cual es incorrecto). `COALESCE(f.costo_flete, 0)` convierte ese valor vacío en `0`, garantizando que la resta funcione perfectamente.
