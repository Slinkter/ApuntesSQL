...
11. **Encontrar clientes cuyo nombre de contacto empieza con 'A':**

    ```sql
    SELECT * FROM customers WHERE contact_name LIKE 'A%';
    ```

12. **Búsqueda insensitive (PostgreSQL):**
    ```sql
...
14. **Seleccionar productos con un precio entre $10 y $20:**
    ```sql
    SELECT * FROM products WHERE unit_price BETWEEN 10 AND 20;
    -- Equivalente a: unit_price >= 10 AND unit_price <= 20
    ```

15. **Seleccionar pedidos de enero 1998 (BETWEEN con fechas):**
    ```sql
...
16. **Encontrar clientes que no tienen especificada una región:**
    ```sql
    SELECT * FROM customers WHERE region IS NULL;
    ```

17. **Encontrar clientes que SÍ tienen región definida:**
    ```sql
...
18. **Clientes de Alemania que viven en Berlin:**

    ```sql
    SELECT * FROM customers WHERE country = 'Germany' AND city = 'Berlin';
    ```

19. **Productos que están descontinuados O tienen 0 unidades en stock:**
    ```sql
    SELECT * FROM products WHERE discontinued = 1 OR units_in_stock = 0;
    ```

20. **(Con paréntesis para claridad) Productos de categoria 1 con precio > 30:**
    ```sql
...
24. **Calcular el precio promedio de los productos:**
    ```sql
...
28. **Calcular el precio promedio de productos por cada categoría (con nombre):**
...
30. **(Con JOIN) Precio promedio y count por nombre de categoría:**
    ```sql
    SELECT 
        c.category_name,
...
31. **INCORRECTO (generará error en PostgreSQL strict):**
    ```sql
...
32. **CORRECTO (agregar la columna al GROUP BY):**
    ```sql
...
33. **CORRECTO (usar aggregate en la columna extra):**
    ```sql
