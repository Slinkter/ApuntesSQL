...
40. **Encontrar todos los productos suministradoados por proveedores de Japón.**
...
50. **INCORRECTO (WHERE después de GROUP BY):**
    ```sql
...
51. **CORRECTO (WHERE antes de GROUP BY - filtra filas):**
    ```sql
    SELECT country, COUNT(*)
    FROM customers
...
49. **(Extra) Contar pedidos por prioridad de envío ('Normal', 'Urgente' si es de USA).**
    ```sql
...
51. **(Extra) Listar los 5 clientes con mayor gasto total.**
    ```sql
...
51. **Asignar un número de fila único a cada producto, ordenado por precio (más caro = 1):**
...
52. **Clasificar productos por precio dentro de cada categoría (RANK vs DENSE_RANK):**
    ```sql
    SELECT 
        product_name, 
        category_id,
...
55. **Mostrar el precio de cada producto junto con el precio promedio de su categoría (sin colapsar):**
...
56. **Ejemplo avanzado: Diferencia del precio vs promedio de categoría**
    ```sql
    SELECT 
        product_name, 
        unit_price,
...
57. **(Extra) Calcular el total de ventas acumulado por mes (PostgreSQL).**
    ```sql
