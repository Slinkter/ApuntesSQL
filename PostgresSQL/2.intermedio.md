...
**Objetivo Académico:** Aplicar restricciones a los resultados de funciones de agregación. A diferencia del `WHERE` (que actúa sobre filas individuales antes del agrupamiento), el `HAVING` actúa sobre los grupos ya formados.
...
**Objetivo Académico:** Aumentar la modularidad y legibilidad del código SQL. Las CTEs definen un conjunto de resultados temporal con nombre que actúa como una "tabla virtual" durante la ejecución de la consulta principal. Es el estándar moderno para reemplazar subconsultas complejas.
...
56. **(Extra) Calcular el total de ventas acumulado por mes (PostgreSQL).**
    ```sql
    ...
    FROM monthly_sales;
    ```
57. **(Extra) Encontrar el producto más vendido en cada categoría.**
    ```sql
